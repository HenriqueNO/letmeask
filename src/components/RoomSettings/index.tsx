...
export function RoomSettings ({roomId}) {
  ...
  useEffect(() => {
    const authorRef = database.ref(`rooms/${roomId}`)

    authorRef.once('value', room => {
      setLengthQuestionValue(room.val().lengthQuestion)
    })
  ...
  function handleInputLenghtQuestion(event) {
    setLengthQuestionValue(event.value)
  }

  function keyUpAndUpgradeDatabase(event) {
    database.ref(`rooms/${roomId}`).update({
      lengthQuestion: Number(event.value)
    })
  }

  return (
    <>
      {!isOpenOverlaySettings === true 
      ? (
        <button onClick={() => setIsOpenOverlaySettings(!isOpenOverlaySettings ? true : false)}>
        ...
        <div className={styles.overlay}>
          ...
          <div className={styles.container}>
            ...
            <div className={styles.caracterConfigContainer}>
              <div>
                <label>Limite de caracteres na pergunta:</label>
                <p>{lengthQuestionValue}</p>
              </div>
              <div>
                <span>25</span>
                <input
                  ...
                  max="300"
                  step="1"
                  defaultValue={lengthQuestionValue}
                  onChange={(e) => handleInputLenghtQuestion(e.target)}
                  onMouseUp={(e) => keyUpAndUpgradeDatabase(e.target)}
